...
def pagamento_sucesso(pedido_id):
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("SELECT status_pagamento FROM pedidos WHERE id = %s", (pedido_id,))
    pedido = cur.fetchone()
    
    if pedido and pedido['status_pagamento'] != 'aprovado':
        ...
        itens = cur.fetchall()
        for item in itens:
            cur.execute("UPDATE produtos SET estoque = estoque - %s WHERE id = %s", (int(item['quantidade']), int(item['produto_id'])))
        conn.commit()
    cur.close(); conn.close()
...
def webhook():
    ...
    if data and data.get('type') == 'payment':
        ...
        if token_row and token_row['valor']:
            mp_token = token_row['valor'].strip()
            ...
            if response.status_code == 200:
                ...
                if status == 'approved' and pedido_id:
                    cur.execute("SELECT status_pagamento FROM pedidos WHERE id = %s", (int(pedido_id),))
                    pedido = cur.fetchone()
                    if pedido and pedido['status_pagamento'] != 'aprovado':
                        cur.execute("UPDATE pedidos SET status_pagamento = 'aprovado' WHERE id = %s", (int(pedido_id),))
                        cur.execute("SELECT produto_id, quantidade FROM itens_pedido WHERE pedido_id = %s", (int(pedido_id),))
                        itens = cur.fetchall()
                        for item in itens:
                            cur.execute("UPDATE produtos SET estoque = estoque - %s WHERE id = %s", (int(item['quantidade']), int(item['produto_id'])))
                        conn.commit()
